...
const ManageBlogEdit = ({data_blog, data_blog_category, data_blog_images}) => {
    const [imagePreviewBanner, setImagePreviewBanner] = useState(data_blog.banner);
    ...
    const { data, setData } = useForm({
        ...
        kategori_blog: data_blog.kategori_blog_id,
        konten: data_blog.konten,
        banner: data_blog.banner,
        status: data_blog.status,
        images: data_blog_images ? data_blog_images.map((img) => img.filename) : []
    });
    ...
    function submit(e) {
        e.preventDefault();
        router.post(`/dashboard/manage-blog/${data_blog.id}`, {
            _method: 'put',
            data_form: data,
        });
    }
                    ...
                    <div className="mt-2 flex items-center gap-x-3">
                        {imagePreviewBanner ? (
                            <div className="w-full h-[200px] rounded-md overflow-hidden border-2 border-primary border-opacity-60">
                                <img
                                    src={imagePreviewBanner.startsWith("data:") ? imagePreviewBanner : `/storage/BlogAssets/Banner/${imagePreviewBanner}`}
                                    alt="Banner Divisi"
